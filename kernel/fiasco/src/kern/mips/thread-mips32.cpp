...
{
  switch (access_once(&utcb->values[0]) & Opcode_mask)
    {
...

  {
    ...
    asm volatile
      (".set push                          \n"
       ...
       "  eret                             \n"
       ".set pop                           \n"
       : : [status] "r" (0x13), [ip] "r" (ip), [sp] "r" (sp), [arg] "r" (a0)
      );
  }
...
thread_guest_tlb_probe(Mword *pfa)
{
  ...
  Vz::State::mfgc0(&gindex, 0, 0);
  Vz::State::mfgc0(&gentryhi, 10, 0);
  Vz::State::mtgc0((*pfa & ~0x3ff) | (gentryhi & 0x3ff), 10, 0);
  asm volatile (".set push; .set virt; ehb; tlbgp; ehb; .set pop");
  Mword index;
  Vz::State::mfgc0(&index, 0, 0);
  if (index & (1UL << 31))
    {
      Vz::State::mtgc0(gindex, 0, 0);
      Vz::State::mtgc0(gentryhi, 10, 0);
      Mem_unit::ehb();
  ...
  Mword gelo[2];
  Mword gpmask;
  Vz::State::mfgc0(&gelo[0], 2, 0);
  Vz::State::mfgc0(&gelo[1], 3, 0);
  Vz::State::mfgc0(&gpmask, 5, 0);
  asm volatile (".set push; .set virt; tlbgr; ehb; .set pop");

  Mword entry;
  Mword mask;
  Vz::State::mfgc0(&mask, 5, 0);
  mask = ((mask | 0x1fff) + 1) >> 1;
  if (*pfa & mask)
    Vz::State::mfgc0(&entry, 3, 0);
  else
    Vz::State::mfgc0(&entry, 2, 0);

  *pfa = (entry & 0x3fffffc0) << 6;

  Vz::State::mtgc0(gelo[0], 2, 0);
  Vz::State::mtgc0(gelo[1], 3, 0);
  Vz::State::mtgc0(gpmask, 5, 0);
  Vz::State::mtgc0(gindex, 0, 0);
  Vz::State::mtgc0(gentryhi, 10, 0);
  Mem_unit::ehb();
  return true;
...
#undef ASM_SEGCTL_ALT
  ...
  else if (!thread_guest_tlb_probe(&pfa))
    {
